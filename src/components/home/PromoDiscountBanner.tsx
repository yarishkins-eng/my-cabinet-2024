...
export default function PromoDiscountBanner() {
  ...
  const { activeDiscount, activeDiscountResolved } = usePromoDiscount();
  ...
  // 🔴 ВЕТВЛЕНИЕ ЯВНОЕ, по порядку, без проваливания. Первая версия этой правки давала
  // «Скидка undefined% активна»: при неотвеченном запросе управление доходило до нижней
  // ветки с пустыми данными. Поймал собственный сторож через минуту после правки.
  //
  // ⚠️ Цена решения названа: если запрос про скидку ОТКАЗАЛ, баннер не покажется вовсе.
  // Это осознанно — не зная, есть ли активная скидка, предлагать забрать новую нельзя:
  // сервер перезаписал бы процент молча. Несовпадение решаем в пользу клиента.
  if (!activeDiscountResolved) return null;
  ...
  // 🔴 Ждём ОТВЕТА про активную скидку, а не просто её отсутствия в кэше. Два запроса летят
  // независимо; ответь список предложений первым — и мы нарисовали бы живую кнопку «забрать
  // 9 %» человеку, у которого уже активны 25 %. Он нажал бы в первые доли секунды, сервер
  // молча перезаписал бы процент и вернул 200 OK: ни ошибки, ни предупреждения, ни отмены.
  // Ровно та потеря, которую этот этап закрывает, — просто пришедшая через время.
  // Воспроизведено скептиком волны 2 отдельным тестом, не выведено рассуждением.
  if (offer && !hasActive) {
