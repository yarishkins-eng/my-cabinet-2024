...
export function CampaignResultsCard({
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-dark-700 bg-dark-800/30 p-4 sm:p-5">
      <div className="mb-4 flex items-center gap-2 sm:gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-dark-300 sm:text-sm">
            ...
            <>
              <div className="flex flex-wrap items-start gap-x-2 gap-y-1">
                <div className="min-w-0 flex-[1_1_9rem] text-start">
                  ...
                  <div
                    dir="ltr"
                    className="truncate text-[10px] text-dark-300 [unicode-bidi:isolate] sm:text-xs"
                  >
                ...
                <div className="ms-auto flex min-w-0 max-w-full items-start gap-1 text-end">
                  <div className="min-w-0">
                    <div className="truncate text-xs font-semibold text-warning-400 sm:text-sm">
                      {formatAmount(campaignReceipts / 100)} {currencySymbol}
                    </div>
                    <div className="truncate text-[10px] text-dark-300 sm:text-xs">
                  ...
                  {canOpenDetails && (
                    <span aria-hidden="true">
                      <ChevronRightIcon
                        className={`mt-0.5 h-4 w-4 shrink-0 text-dark-300 ${isRtl ? 'rotate-180' : ''}`}
                      />
                    </span>
                  )}
                </div>
              ...
              {hasReceiptContract && (
                <div className="mt-2">
                  <div className="mb-1 text-start text-[10px] leading-snug text-dark-300 sm:text-xs">
                    {shareLabel(share)}
                  </div>
                  <div
                    className="h-1 overflow-hidden rounded-full bg-dark-700/70"
                    aria-hidden="true"
                  >
                    <div
            ...
            <li key={campaign.id}>
              {canOpenDetails ? (
                <Link
                  ...
                  className="group block min-h-11 rounded-lg bg-dark-900/50 p-2 transition-colors hover:bg-dark-800/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 sm:p-3"
                >
                ...
                <div className="min-h-11 rounded-lg bg-dark-900/50 p-2 sm:p-3">{content}</div>
              )}
            </li>
          );
        })}
      ...
      <div className="mt-auto pt-4">
        {hasReceiptContract && (
          ...
          >
            <div className="rounded-lg bg-dark-900/50 p-2">
              <dt className="text-[10px] text-dark-300 sm:text-xs">
                {t('adminDashboard.topCampaigns.perLead')}
              </dt>
              <dd className="mt-0.5 text-xs font-semibold text-dark-100 sm:text-sm">
        ...
        <div className="border-t border-dark-700 pt-4">
          <div className="flex items-start justify-between gap-3">
            <span className="text-xs text-dark-300 sm:text-sm">
          ...
          <p className="mt-2 text-[10px] leading-relaxed text-dark-300 sm:text-xs">
