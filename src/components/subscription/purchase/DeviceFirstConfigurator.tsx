...
export function DeviceFirstConfigurator({
  ...
  useEffect(() => {
    ...
    if (fixtureCheckout === undefined && !initialCheckoutId) {
      ...
      setExistingPaymentAttempt(null);
      setConfirmAbandon(false);
      // 🔴 МИНА EW, ТРЕТИЙ РАЗ, и нашёл её скептик волны 2 живым прогоном. Мест сброса
      // удержания было ДВА — «Изменить параметры» и уход с подтверждения, — и оба честно
      // держали пару «флаг + замороженный ключ». Но есть третий выход, не аудированный никем:
      // системная «назад» вебвью после того, как заказ уже оплачен и закрыт. Он чистил всё
  ...
  // 🔴 РЕК-3: у выбора появился ТРЕТИЙ признак — намерение сразу открыть подтверждение.
  // Он живёт ВНУТРИ той же записи, а не отдельным флагом, и это не украшательство: отдельный
  // флаг пережил бы посев, которому не нашлось цены, и позже открыл бы подтверждение на
  // ЧУЖОМ выборе — том, что кладёт сюда `rememberSelection` при закрытии заказа.
  ...
  // ⛔ И подтверждение НЕ открываем ЗДЕСЬ, хотя этап РЕК-3 именно им и заканчивается: гард
  // эффекта синхронизации первой строкой делает `if (confirmation || …) return`, то есть на уже
  // открытом подтверждении выбор не применился бы НИКОГДА, и на денежном экране человек увидел
  // бы «Недоступно» сразу после успешной доплаты. Поэтому здесь только НАМЕРЕНИЕ (`confirm`),
  ...
  // просто кадром позже.
  // ⚠️ ЧЕСТНО: САМ КАДР МИГАНИЯ СТОРОЖЕМ НЕ ЗАКРЫТ и закрыт быть не может — в jsdom эффекты
  // успевают отработать внутри `render`, промежуточного состояния тест не видит. Мутационный
  // прогон это подтвердил: снятие признака переживает весь набор. Закрыта только ОПАСНАЯ
  // сторона — «признак завис истинным и экран выбора не появился никогда»: её ловят 55 тестов.
  // ⛔ Не снимать признак на основании «мутация переживает»: цена его отсутствия названа выше
  // и проверена глазами, а не тестом.
  ...
  // 🔴 РЕК-18. Шаг нужен снаружи денежного блока: на подтверждении шапка тарифа сжимается,
  // а его описание уезжает ПОД кнопки. На экране выбора и то и другое остаётся как было —
  // там описание продаёт и месту не мешает. Разводим по шагу, а не прячем везде.
  const isConfirmationStep = Boolean((confirmation || resumedConfirmation) && !legacyDraft);
  // 🔴 РЕК-18, попутная починка из волны. Переход к подтверждению подменяет ветку ВНУТРИ той
  // же секции, и прокрутка не сбрасывается ничем: человек жмёт «Перейти к оформлению» внизу
  // длинного списка сроков и остаётся на той же координате — то есть на новом экране сразу
  // оказывается посередине. Это складывалось с 267 px и давало «кнопка болтается где-то там».
  // ⚠️ Двигаем ТОЛЬКО когда действительно прокручены: иначе это лишний вызов на каждом входе.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (preferred && priceFor(preferred.period, preferred.devices)) {
      ...
      // 🔴 РЕК-3, приземление после доплаты. Открываем подтверждение ровно ЗДЕСЬ и больше нигде:
      // это единственная точка, где посеянный выбор уже применён И у него нашлась цена. Открой
      // мы его раньше — гард четырьмя строками выше вышел бы первым, выбор не применился бы
      // никогда, и человек прочитал бы «Недоступно» сразу после успешной доплаты.
      // ⛔ Ветка «цены нет» сюда не попадает намеренно: там человек остаётся на экране выбора с
      // рабочим умолчанием, и подтверждение с пустым «Итого» было бы хуже лишнего нажатия.
      // ⚠️ `autostartHeldForWallet` тут НЕ трогаем и трогать нечего: на дороге возврата экран
      // рождается заново, флаг у него `false` от рождения. Плашка «Мы не открыли оплату» на этом
      // приземлении соврала бы — никто ничего не удерживал (класс мины EW), на это есть сторож.
  ...
  //  · `methods.isSuccess` — вход РЕАЛЬНЫЙ и доказан экспериментом. ⚠️ Здесь стояла моя
  //    оговорка «забор структурный, состояние в тестах не воспроизводится» — скептик волны 2
  //    её ОПРОВЕРГ дампом состояния запроса: после упавшего перезапроса `status: 'error'` при
  //    живых `data`, экран красит ветку отказа, и кнопка доплаты в среднем слоте исчезает.
  //    Пережила не мутация, а мой сторож: он проверял кнопку ДО перерисовки. Сторож починен;
  ...
  const topUpAction = showTopUpAction ? (
    <div className="space-y-1">
      ...
      {/* Подпись существует только там, где есть чем доплачивать и куда возвращаться.
          ...
          нет ни в одной из четырёх локалей. Новый текст не называет кнопок вовсе: обещает ровно
          то, что этап исполняет, — выбор сохранится, останется оформить заказ.
          🔴 Этап РЕК-3 сделал часть этого обоснования устаревшей, и она правится здесь, а не
          остаётся врать: ~~«после В-1 человек вдобавок возвращается не „сюда“, а на экран
          выбора»~~ — теперь он возвращается ИМЕННО сюда, на подтверждение, и обещание строки
          («останется оформить заказ») исполняется буквально одним нажатием.
          ...
          {payMutation.isPending || nativeLaunchMutation.isPending ? (
            ...
            <>
              ...
              {autostartHeldForWallet && (
                <div
                  role="status"
                  className="rounded-xl border border-accent-400/40 bg-accent-500/10 px-4 py-2.5"
                >
                  {/* 🔴 РЕК-18.2: заголовок и первое предложение тела говорили одну мысль двумя
                  ...
                  <p className="text-sm text-dark-100">
                    ...
                    {/* 🔴 Мина JY, нашла ревизия: разделитель стоял БЕЗУСЛОВНО, а тело плашки
                        при незагруженном минимуме пусто — на первом кадре денежного экрана
                        висел знак в никуда. Достаётся он ровно той когорте, ради которой всё
                ...
                <>
                  ...
                  {methods.isLoading && !chatChosenMethod ? (
                    ...
                    <>
                      ...
                      {/* 🔴 РЕК-18.5. Здесь стояли ДВА абзаца подряд: оговорка про баланс и
                          ...
                          ⛔ Цвет `text-dark-300`, а НЕ `-400`: на 12 px `-400` даёт контраст
                          ниже нормы 4,5 (история: РС-14д). Этот же дефект в этом файле уже
                          чинили однажды — комментарий об этом стоит у сводки ниже. */}
                      <p className="text-xs text-dark-300">
                        {hasWallet && (
                          <span>
                            {/* 🔴 Прогон сценария и линза денег независимо: главное следствие
                              решения владельца в том, что человек может нажать залитую кнопку
                              своего способа и переплатить на весь свой баланс, а строка про
                              «не спишутся» была НЕЙТРАЛЬНОЙ — она сообщала факт и не называла
                              цену. Называем цену числом. */}
                            {chatChosenMethod
                              ? t('deviceFirst.payFullAmountNotice', {
                                  balance: formatPrice(confirmBalanceKopeks ?? 0),
                                })
                              : t('deviceFirst.paymentMethodsAvailable')}
                          </span>
                        )}
                        {hasWallet ? ' · ' : ''}
                        <span>{t('deviceFirst.twoStepPayHint')}</span>
                      </p>
              ...
              <button
                ...
                // теме, то есть ниже порога читаемости. Пока человек
              ...
              {/* 🔴 РЕК-18.4. Описание тарифа переехало сюда с самого верха экрана. Оно НЕ
                  удалено: на шаге выбора оно по-прежнему стоит развёрнутым и продаёт. Здесь,
                  на денежном шаге, оно уходит под свёртку — и обязательно НИЖЕ кнопок:
                  замер показал, что раскрытая свёртка добавляет 112 px, и стой она выше,
      ...
      {balanceKopeks !== null && (
        <div>
          ...
          {/* 🔴 РЕК-14.2. Человек, вернувшийся из банка, задаёт себе один вопрос — «мои деньги
              ...
              делала — и отступ был МЁРТВЫМ: у `.space-y-1.5 > * ~ * (до РЕК-18 было `space-y-3`)` специфичность выше, чем у
              ...
              ⚠️ `text-dark-300`, а не `-400`: `-400` при 12 px даёт контраст ниже порога
              читаемости. Ту же цену этот файл уже платил этапом раньше.
