      ...
      {checkout?.ui_state === 'awaiting_payment' && (
        ...
        >
          ...
          {/* 🔴 ПУНКТ 2б: БАННЕР ОТКАЗА НЕ ИМЕЕТ ПРАВА ПЕРЕКРИКИВАТЬ СЕРВЕР.
              ...
              ⛔ `canPayNow` — это и есть голос сервера, а не просто «есть ли кнопка»: адрес
              оплаты гаснет в `_is_live_direct_provider_invoice` (`bot-code/app/cabinet/routes/
              device_first.py:161-186`) ровно тогда, когда попытка ушла в сверку либо платёж
              уже помечен оплаченным. То есть `!canPayNow` = «сервер знает про эти деньги
              больше, чем редирект», и объявлять в этот момент отказ — врать про деньги.
              ...
              Текст при этом ссылается на провайдера («Платёжная система сообщила»), а не
              утверждает от нашего имени. */}
