...
export function DeviceFirstConfigurator({
  ...
  //  · `methods.isSuccess` — забор СТРУКТУРНЫЙ, и это надо назвать честно. Две линзы волны 1
  //    независимо описали вход «способы загружены, повторный запрос упал: экран рисует ветку
  //    отказа, а кнопка доплаты уезжает в средний слот, которого в той ветке нет». Я написал на
  //    него сторож — и мутация «снять этот забор» ПЕРЕЖИЛА весь набор, то есть в тестах это
  //    состояние воспроизвести не удалось: при живых данных запрос не переходит в `error`.
  //    Значит найденный вход экспериментом НЕ подтверждён. Забор оставлен потому, что делает
  //    инвариант верным независимо от поведения библиотеки, а не потому, что доказан;
