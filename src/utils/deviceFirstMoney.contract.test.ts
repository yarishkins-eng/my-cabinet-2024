...
describe('вердикт о деньгах доведён до всех экранов', () => {
  ...
  it('мина W: предупреждение перед уходом говорит и про возврат, и про судьбу заказа', () => {
    ...
    expect(locale('ru').deviceFirst.endsAt).toBe('Подписка до');
    expect(locale('ru').deviceFirst.invoiceReadyUntilText).toContain('Позже заказ придётся');
    expect(ru).toContain('отдельным окном');
    expect(ru).toContain('останется');
    expect(ru).not.toContain('поверх кабинета');
    expect(ru).not.toContain('обновится сам');
