...
describe('TopUpAmount — короткий путь кассы', () => {
  ...
  // 🔴 РЕК-20, сторож на ОТСУТСТВИЕ блока — его прямо требует пункт плана, и он не декоративный.
  // `TopUpAmount.tsx` активно ведёт апстрим: из 22 коммитов 14 чужих. Обновление разработчиков
  // может молча вернуть кнопки готовых сумм на денежный экран, и весь набор останется зелёным —
  // потому что проверять их отсутствие было нечем. Фикстура нарочно ОТДАЁТ `quick_amounts`
  // (`[10000, 30000]`), то есть сервер по-прежнему присылает суммы; экран обязан их не рисовать.
  it('never offers ready-made amounts: the person already has his number', async () => {
    renderScreen('', { warmCache: [platega] });
    await settle();

    // Улика, что экран поднялся: поле суммы на месте.
    expect(screen.getByRole('spinbutton')).toBeTruthy();
    // А кнопок с суммами из ответа сервера нет ни одной.
    for (const preset of ['100', '300']) {
      expect(
        screen.queryByRole('button', { name: new RegExp(`^\\s*${preset}\\b`) }),
        `кнопка готовой суммы «${preset}» вернулась на денежный экран`,
      ).toBeNull();
    }
  });
  ...
  // 🔴 Нашёл критик полноты: обработчиков, меняющих сумму, было ТРИ. Быстрая кнопка оставляла живой
  ...
  // Защита осталась та же: обработчиков, гасящих живой счёт при смене суммы, теперь ДВА —
  // поле и чип способа, — и ввод в поле один из них (`TopUpAmount.tsx`, `onChange` делает `setPaymentUrl(null)` вслед за
