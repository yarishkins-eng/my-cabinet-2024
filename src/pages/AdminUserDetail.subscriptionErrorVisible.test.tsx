...
describe('Отказ сервера на форме подписки доходит до владельца (пункт 2.2б)', () => {
  ...
  // 🔴 ПРОБЕЛ, ЗАПИСАН ЧЕСТНО (в план не заводится — ревизия 20.08). Мутация «выкинуть tariff_id из тела
  ...
  it('претензии валидации списком не роняют экран, а читаются словами', async () => {
    ...
    // Живой вход сегодня один: дробное число в поле «дней». Путь «больше 3650»
    // закрыт зажимом ввода — но зажим это удобство, а защита здесь разборщик:
    // снятие зажима не заметит ни один тест, снятие разборщика роняет этот.
    getUser.mockResolvedValue(USER);
