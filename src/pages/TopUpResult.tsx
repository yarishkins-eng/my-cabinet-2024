  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {/* ⚠️ Цвет НЕ трогаем намеренно: янтарные токены (`warning-*`) на этом фоне уже
            давали нечитаемый контраст — на этом обожглись этапом РС-14д. Заметность даёт
            не цвет, а знак в начале строки (так же, как в чате) и сменившаяся главная кнопка. */}
...
export default function TopUpResult() {
  ...
  useEffect(() => {
    ...
    if (cleanedUpRef.current) return;
    {
      ...
      // 🔴 РЕК-3: ПОМЕТИТЬ протухшим было МАЛО, и это нашли три линзы ревью независимо.
      // `invalidateQueries` по умолчанию перезапрашивает только АКТИВНЫЕ запросы, а кассы в
      // этот момент на экране нет — запись оставалась в кэше (`gcTime` 5 минут, а оплата по
      // СБП укладывается в минуту). На следующем монтировании данные отдаются из кэша
      // синхронно, `isLoading` при этом ложно, заслон загрузки не держит — и человек видел
      // СТАРЫЙ баланс. Пока приземление вело на экран выбора, это было невидимо: там баланса
      // нет вовсе. С приземлением на подтверждение тот же кадр стал денежным: «Не хватает
      // 199 ₽» и залитая кнопка «Доплатить 199 ₽» — ровно та сумма, которую человек только
      // что заплатил. Нажмёт — заплатит второй раз (`TopUpAmount` создаёт счёт сам по `auto=1`).
      // `removeQueries` даёт то, чего не даёт пометка: кэша нет ⇒ `isLoading` истинно ⇒ экран
      // покупки держит спиннер и рисует деньги ТОЛЬКО из свежего ответа сервера.
      // ⛔ Не менять обратно на `invalidateQueries`: на этом пути наблюдателя нет, и «фоновое
      // обновление» тут означает «показать вчерашнее число, а потом подменить его под пальцем».
      // ⚠️ ЧЕСТНАЯ ГРАНИЦА, названа скептиком: это лечит дорогу ЧЕРЕЗ ЭТОТ ЭКРАН. Мимо него
      // деньги приходят ещё двумя каналами — вебсокетом (там ключ дописан, см.
      // `WebSocketNotifications.tsx`) и забором промо-скидки (`PromoDiscountBanner.tsx`, там
      // по-прежнему только пометка). Утверждать «касса физически не может нарисовать старый
